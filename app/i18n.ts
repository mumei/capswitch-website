...
export const translations: Record<Locale, Translation> = {
  ja: {
    ...
    tour: {
      ...
      steps: ["Caps Lockを使っていなかった", "Fnを押しながらの操作が面倒だった", "Caps Lockをスイッチにした"],
      descriptions: [
        "出番のないキーとLEDを 有効活用したい",
        "F1〜F12とメディア操作を使い分けるたび Fnを押す手間をなくしたい",
        "押すだけで切替 LEDで状態を確認",
      ],
  ...
  en: {
    ...
    tour: {
      ...
      steps: ["Caps Lock went unused", "Holding Fn for the other F1–F12 action was tedious", "Caps Lock became the switch"],
      descriptions: ["Put the unused key and its LED to work", "Stop holding Fn when choosing between media controls and function keys", "Switch with one press and see the state on the LED"],
  ...
  de: {
    ...
    tour: {
      ...
      steps: ["Caps Lock blieb ungenutzt", "Fn für die andere F1–F12-Belegung zu halten war umständlich", "Caps Lock wurde zum Schalter"],
      descriptions: ["Ungenutzte Taste und LED sinnvoll einsetzen", "Beim Wechsel zwischen Mediensteuerung und Funktionstasten nicht mehr Fn halten", "Mit einem Tastendruck wechseln und den Status an der LED sehen"],
  ...
  "zh-Hans": {
    ...
    tour: {
      ...
      steps: ["Caps Lock 一直闲置", "按住 Fn 使用另一组 F1–F12 功能很麻烦", "让 Caps Lock 成为切换开关"],
      descriptions: ["让闲置按键和 LED 派上用场", "在媒体控制与功能键之间切换时 不再需要按住 Fn", "按一下即可切换并通过 LED 查看状态"],
  ...
  "zh-Hant": {
    ...
    tour: {
      ...
      steps: ["Caps Lock 一直閒置", "按住 Fn 使用另一組 F1–F12 功能很麻煩", "讓 Caps Lock 成為切換開關"],
      descriptions: ["讓閒置按鍵和 LED 派上用場", "在媒體控制與功能鍵之間切換時 不再需要按住 Fn", "按一下即可切換並透過 LED 查看狀態"],
  ...
  fr: {
    ...
    tour: {
      ...
      steps: ["Caps Lock restait inutilisée", "Maintenir Fn pour l’autre action F1–F12 était gênant", "Caps Lock est devenue le commutateur"],
      descriptions: ["Mettre la touche inutilisée et sa LED à profit", "Ne plus maintenir Fn pour passer des commandes média aux touches de fonction", "Basculer d’un appui et voir l’état sur la LED"],
  ...
  ko: {
    ...
    tour: {
      ...
      steps: ["Caps Lock을 쓰지 않았다", "F1–F12의 다른 기능을 쓸 때마다 Fn을 눌러야 했다", "Caps Lock을 스위치로 만들었다"],
      descriptions: ["놀고 있던 키와 LED를 제대로 활용하기", "미디어 조작과 Function 키를 오갈 때 Fn을 누르는 과정 없애기", "한 번 눌러 전환하고 LED로 상태 확인하기"],
  ...
  es: {
    ...
    tour: {
      ...
      steps: ["Caps Lock no se usaba", "Mantener Fn para la otra acción de F1–F12 era incómodo", "Caps Lock se convirtió en el selector"],
      descriptions: ["Aprovechar la tecla sin uso y su LED", "Dejar de mantener Fn al alternar entre controles multimedia y teclas de función", "Cambiar con una pulsación y ver el estado en el LED"],
  ...
  it: {
    ...
    tour: {
      ...
      steps: ["Caps Lock rimaneva inutilizzato", "Tenere premuto Fn per l’altra azione F1–F12 era scomodo", "Caps Lock è diventato il selettore"],
      descriptions: ["Sfruttare il tasto inutilizzato e il suo LED", "Non dover più tenere Fn passando tra controlli multimediali e tasti Funzione", "Cambiare con una pressione e vedere lo stato sul LED"],
  ...
  vi: {
    ...
    tour: {
      ...
      steps: ["Caps Lock gần như không được dùng", "Phải giữ Fn để dùng chức năng F1–F12 còn lại thật bất tiện", "Caps Lock trở thành công tắc"],
      descriptions: ["Tận dụng phím và đèn LED đang bỏ trống", "Bỏ thao tác giữ Fn khi đổi giữa điều khiển media và phím chức năng", "Nhấn một lần để chuyển và xem trạng thái trên LED"],
  ...
  th: {
    ...
    tour: {
      ...
      steps: ["Caps Lock แทบไม่ได้ใช้", "การกด Fn ค้างเพื่อใช้อีกหน้าที่ของ F1–F12 ยุ่งยาก", "Caps Lock กลายเป็นสวิตช์"],
      descriptions: ["ใช้ปุ่มและไฟ LED ที่ว่างอยู่ให้เกิดประโยชน์", "ไม่ต้องกด Fn ค้างเมื่อสลับระหว่างการควบคุมสื่อกับปุ่มฟังก์ชัน", "กดครั้งเดียวเพื่อสลับและดูสถานะจากไฟ LED"],
